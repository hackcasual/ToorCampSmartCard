...
try:
    ...
    response, sw1, sw2 = cardservice.connection.transmit( enter_pin('654321') )

    response, sw1, sw2 = cardservice.connection.transmit( encrypt( [x for x in range(0,16)] ) )
    encrypted_blob = response[0:16]

    response, sw1, sw2 = cardservice.connection.transmit( decrypt( encrypted_blob ) )
